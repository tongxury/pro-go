...
type VideoReplication3_SegmentScriptJob struct {
	data *data.Data
}

func (t VideoReplication3_SegmentScriptJob) Initialize(ctx context.Context, options Options) error {
	return nil
}

func (t VideoReplication3_SegmentScriptJob) GetName() string {
	return "segmentScriptJob"
}

func (t VideoReplication3_SegmentScriptJob) Execute(ctx context.Context, jobState *projpb.Job, wfState *projpb.Workflow) (status *ExecuteResult, err error) {

	dataBus := GetDataBus(wfState)

	logger := log.NewHelper(log.With(log.DefaultLogger,
		"func", "VideoReplication3_SegmentScriptJob.Execute",
