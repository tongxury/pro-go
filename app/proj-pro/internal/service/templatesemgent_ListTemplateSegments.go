...
func (t ProjService) ListTemplateSegments(ctx context.Context, params *projpb.ListResourceSegmentsRequest) (*projpb.ResourceSegmentList, error) {

	size := helper.Select(params.Size > 0, params.Size, 24)
	page := helper.Select(params.Page > 0, params.Page, 1)

	if params.Keyword != "" {
		list, err := t.searchTemplateSegments(ctx, params.Keyword, params.SearchBy, int(params.Size))
		if err != nil {
			return nil, err
		}
...
func (t ProjService) searchTemplateSegments(ctx context.Context, keyword, by string, size int) ([]*projpb.ResourceSegment, error) {
