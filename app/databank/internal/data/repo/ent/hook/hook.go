...
// The UserAssetFunc type is an adapter to allow the use of ordinary
// function as UserAsset mutator.
type UserAssetFunc func(context.Context, *ent.UserAssetMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserAssetFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UserAssetMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserAssetMutation", m)
}
