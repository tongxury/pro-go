...
func (t *Client) DeleteByRequest(ctx context.Context, index string, params deletebyquery.Request) (*deletebyquery.Response, error) {
	return t.c.DeleteByQuery(index).Request(&params).Do(ctx)
}
...
//func (t *Client) Search(ctx context.Context, params SearchParams) (*types.HitsMetadata, error) {
//	s := t.c.Search().Index(params.Index)
//
//	if len(params.MatchQueries) > 0 {
//		s.Request(&search.Request{
//			Query: &types.Query{
//				Match: params.MatchQueries,
//			},
//		})
//	}
//
...
func (t *Client) Search(ctx context.Context, params SearchParams) (*types.HitsMetadata, error) {
	...
	for k, v := range params.MatchQueries {
		qs = append(qs, types.Query{
	...
	s = s.Request(&search.Request{
		Query: &types.Query{
			Bool: &types.BoolQuery{
				Should:             qs,
				MinimumShouldMatch: helper.Select(params.MinMatches == 0, len(params.MatchQueries), params.MinMatches),
			},
		},
		Size: helper.Select(params.Size > 0, &params.Size, nil),
	})
...
func (t *Client) SearchV2(ctx context.Context, index string, params search.Request) (*types.HitsMetadata, error) {
...
func (t *Client) SearchById(ctx context.Context, index string, id string) (*types.HitsMetadata, error) {
	s := t.c.Search().Index(index)

	s = s.Request(&search.Request{Query: NewTermQuery("_id", id)})
